...
namespace client_cssom::css_parser
{
  ...
  private:
    explicit CSSImageParser(const std::vector<css_tokenizer::Token>& tokens);
    ...
    std::vector<values::generics::GenericGradientItem<values::specified::Color, 
                                                      values::specified::LengthPercentage>> parseColorStops();
    values::generics::GenericGradientItem<values::specified::Color, 
                                          values::specified::LengthPercentage> parseColorStop();
    
    // Helper methods
    bool consumeToken(css_tokenizer::TokenType expected_type);
    bool consumeIdentifier(const std::string& expected_value);
    bool consumeFunction(const std::string& expected_name);
    bool hasNext() const;
    const css_tokenizer::Token& currentToken() const;
    const css_tokenizer::Token& peekToken(size_t offset = 1) const;
    void advance();
    void skipWhitespace();
    
    std::vector<css_tokenizer::Token> tokens_;
    size_t position_;
  };
}
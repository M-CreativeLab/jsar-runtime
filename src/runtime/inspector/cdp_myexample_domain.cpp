...
std::string CdpMyExampleDomain::handleMethod(const std::string &method, const CdpMessage &message)
{
  ...
  else
  {
    ...
    return CdpResponse::error(message.id, -32601, "MyExample." + method + " is not supported");
  }
}
...
std::string CdpMyExampleDomain::getInfo(const CdpMessage &message)
{
  ...
  result.AddMember("domain", rapidjson::Value().SetString("MyExample", allocator), allocator);

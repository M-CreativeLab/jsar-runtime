...
std::unique_ptr<CdpMessage> CdpMessage::parse(const std::string& json) {
...
std::string CdpHandler::processMessage(const std::string& message) {
    ...
    std::string domain = extractDomain(cdpMessage->method);
    std::string methodName = extractMethodName(cdpMessage->method);
    
    DEBUG(LOG_TAG_INSPECTOR, "CDP: Domain=%s, Method=%s, ID=%d", 
          domain.c_str(), methodName.c_str(), cdpMessage->id);
...
void CdpHandler::registerDomain(const std::string& domain, std::unique_ptr<CdpDomainHandler> handler) {
    ...
    domains_[domain] = std::move(handler);
}

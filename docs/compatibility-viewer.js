class JSARCompatibilityViewer {
    ...
    renderAPIGroup(groupName, apis) {
        ...
        return `
            <div class="api-item bg-white rounded-lg shadow-sm ${categoryColor} overflow-hidden" style="border-left: 4px solid;">
                <div class="p-6">
    ...
    renderAPIItem(api) {
        ...
        const versionBadge = `<span class="badge bg-jsar text-white">v${api.version}</span>`;
        
        return `
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded" style="padding: 0.75rem;">
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                        <code class="text-sm font-mono text-gray-800">${api.name}</code>
                        ${statusBadge}
                        ${versionBadge}
                    </div>
                    <div class="flex gap-2 text-xs">
                        ${api.mdnUrl ? `<a href="${api.mdnUrl}" target="_blank" class="text-blue-600">MDN Docs</a>` : ''}
                        ${api.specUrl ? `<a href="${api.specUrl}" target="_blank" class="text-green-600">Specification</a>` : ''}
                    </div>
                </div>
            </div>
        `;
    ...
    getCategoryColor(category) {
        switch (category) {
            case 'api': return 'border-blue-500';
            case 'html': return 'border-green-500';
    ...
    getStatusBadge(api) {
        if (api.deprecated) {
            return '<span class="badge bg-red-100 text-red-800">Deprecated</span>';
        }
        if (api.experimental) {
            return '<span class="badge bg-yellow-100" style="color: #92400e;">Experimental</span>';
        }
        if (api.standardTrack) {
            return '<span class="badge bg-green-100 text-green-800">Stable</span>';
        }
        return '';
    }
    ...
    showError() {
        document.getElementById('loading').innerHTML = `
            <div class="text-center py-12">
                <div class="text-red-400" style="font-size: 4rem; margin-bottom: 1rem;">⚠️</div>
                <h3 class="text-lg font-medium text-gray-900 mb-2">Failed to load compatibility data</h3>
                <p class="text-gray-600">Please check the console for more details</p>
            </div>
        `;
    }
}

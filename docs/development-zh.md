...
JSAR 提供了一个专门的 GPU 调试工具，用于实时监控和调试渲染器的命令缓冲区执行、性能指标和 GPU 资源使用情况。

### 启动调试工具

1. **启用 Inspector 支持**

   确保编译时启用了 inspector 支持：
...
2. **启动 JSAR 运行时**
   ...
   启动 JSAR 运行时后，Inspector 服务器将自动在端口 9423 上启动并提供 CDP (Chrome DevTools Protocol) 支持。
...
3. **打开调试界面**
   ...
   或者将文件部署到本地 HTTP 服务器上访问。

### 连接到渲染器

1. 在调试界面中点击 "Connect" 按钮
2. 工具将自动连接到 `ws://localhost:9423/devtools/inspector/1`
3. 连接成功后，状态指示器将变为绿色
...
- **启用追踪**: 点击 "Enable Tracing" 按钮开始监控命令缓冲区
- **禁用追踪**: 点击 "Disable Tracing" 按钮停止监控
- **实时命令流**: 启用追踪后可以看到实时的 OpenGL/WebGL 命令执行

#### 2. 帧率控制

- **设置客户端帧率**: 使用 FPS 控制器调整目标帧率（1-120 FPS）
- **实时 FPS 显示**: 在性能指标面板中查看当前帧率

#### 3. 事件节流控制

为了避免过多的事件占用带宽和影响性能，工具提供事件节流功能：

- **设置事件频率**: 使用 "Event Throttle" 控制器调整事件发送频率（0.1-100 events/sec）
- **性能统计**: 实时查看接收到的事件数量和丢弃率
...
| 方法名称 | 参数 | 描述 |
|---------|------|------|
| `enableTracing` | 无 | 启用渲染追踪 |
| `disableTracing` | 无 | 禁用渲染追踪 |
| `setClientFrameRate` | `frameRate`: 1-120 | 设置客户端目标帧率 |
| `getRendererInfo` | 无 | 获取渲染器状态信息 |
| `getContentRenderers` | 无 | 获取所有内容渲染器列表 |
| `getCommandBuffers` | 无 | 获取命令缓冲区调试信息 |
| `setEventThrottle` | `eventsPerSecond`: 0.1-100 | 设置事件节流频率 |

### 故障排除

**工具无法连接到渲染器**
- 确保 JSAR 运行时已启动且启用了 Inspector 支持
- 检查端口 9423 是否被占用
- 确认防火墙设置允许本地连接

**看不到命令缓冲区事件**
- 确保已点击 "Enable Tracing" 启用追踪
- 检查是否有应用正在运行并产生渲染命令
- 调整事件节流设置，增加事件频率

...
### Configure XR click distance threshold

To configure the click distance threshold for XR device ray interactions:

```sh
adb shell setprop jsar.xr.click.distance.threshold 5
```

This controls the maximum distance (in pixels) between `mousedown` and `mouseup` hit points for triggering click events. The default value is 5 pixels, which provides good tolerance for XR ray jitter while maintaining distinction between click and drag gestures.

- **Smaller values** (1-3): More precise click detection, may be harder to trigger clicks with XR devices
- **Larger values** (5-10): More tolerant of ray jitter, easier to trigger clicks but may register drags as clicks

You can also set this via environment variable:

```sh
export JSAR_XR_CLICK_DISTANCE_THRESHOLD=5
```

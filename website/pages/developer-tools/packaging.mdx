...
As shown above, right-click in the empty space of your project, and select "JSAR: Package" to package the project. Afterward, you'll find a `{name}-{version}.zip` file in the current directory, where `name` and `version` are the name and version specified in your `package.json` file.

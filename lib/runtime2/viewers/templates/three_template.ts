  ...
  <script type="module">
    ...
    navigator.xr.requestSession('immersive-ar', {
      ...
      function handelIntersection() {
        ...
        const point = intersectObjects(controller, raycaster, group);
        if (!isSelecting || !selectedObject || !point) return;

        // 重用向量对象计算位置差
        _deltaPosition.copy(point).sub(initialControllerPosition);

        const rotationSpeed = 2.0;
        const maxRotation = Math.PI / 2; // 限制最大旋转角度
        const deltaRotationX = Math.min(maxRotation, _deltaPosition.y * rotationSpeed);
        const deltaRotationY = Math.min(maxRotation, -_deltaPosition.x * rotationSpeed);
        ...
        // 重用欧拉角对象
        _euler.setFromQuaternion(_targetQuaternion, 'XYZ');
        _initialEuler.setFromQuaternion(initialObjectQuaternion, 'XYZ');
        _euler.z = _initialEuler.z; // 锁定Z轴旋转

import { IWebXRFeature, WebXRFeatureName, WebXRFeaturesManager } from '../FeatureManager';
...
import { IWebXRMotionControllerAxesValue, WebXRControllerComponent } from '../motionController';

type Nullable<T> = BABYLON.Nullable<T>;
type Observer<T> = BABYLON.Observer<T>;
type Observable<T> = BABYLON.Observable<T>;
...
type LinesMesh = BABYLON.LinesMesh;
type PickingInfo = BABYLON.PickingInfo;

const {
  Ray, UtilityLayerRenderer, StandardMaterial,
  Color3, Color4, Vector3, Curve3, Quaternion, Axis,
  Observer, Observable,
  PointerEventTypes,
  DynamicTexture,
  SineEase, EasingFunction, Animation,
  setAndStartTimer,
  CreateCylinder, CreateTorus, CreateGround, CreateLines,
} = BABYLON;
...
export interface IWebXRTeleportationOptions {
   ...
   */
  customUtilityLayerScene?: BABYLON.Scene;
  /**
   ...
   */
  generateRayPathMesh?: (points: Vector3[], pickingInfo: BABYLON.PickingInfo) => AbstractMesh;
}
...
export class WebXRMotionControllerTeleportation extends WebXRAbstractFeature {
   ...
   */
  public onTargetMeshPositionUpdatedObservable: Observable<BABYLON.PickingInfo> = new Observable();

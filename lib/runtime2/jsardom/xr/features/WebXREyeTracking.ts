...
const { TmpVectors, Vector3, Ray } = BABYLON;
...
export class WebXREyeTracking extends WebXRAbstractFeature {
  private _latestEyeSpace: BABYLON.Nullable<XRSpace>;
  private _gazeRay: BABYLON.Nullable<BABYLON.Ray>;
   ...
   */
  public readonly onEyeTrackingStartedObservable: BABYLON.Observable<BABYLON.Ray> = new BABYLON.Observable();
  /**
   * This observable will notify registered observers when eye tracking ends
   */
  public readonly onEyeTrackingEndedObservable: BABYLON.Observable<void> = new BABYLON.Observable();
  /**
   * This observable will notify registered observers on each frame that has valid tracking
   */
  public readonly onEyeTrackingFrameUpdateObservable: BABYLON.Observable<BABYLON.Ray> = new BABYLON.Observable();
   ...
   */
  public getEyeGaze(): BABYLON.Nullable<BABYLON.Ray> {
    return this._gazeRay;
  }
  ...
  protected _onXRFrame(frame: XRFrame) {
    ...
    if (this._latestEyeSpace && this._gazeRay) {
      ...
      if (pose) {
        this._gazeRay.origin
          .set(pose.transform.position.x, pose.transform.position.y, pose.transform.position.z)
